...
static gboolean mptcp_interdsn_latency              = FALSE;
...
/* print delta delay between current data and the packet which sent the byte just before */
static void
mptcp_dsn_add_latency(packet_info *pinfo,  tvbuff_t *tvb,
    proto_tree *tree, struct tcp_analysis* tcpd, struct tcpheader * tcph, mptcp_per_packet_data_t* mptcppd)
{

    nstime_t *highest_time = &(pinfo->fd->abs_ts);
    mptcp_dsn2packet_mapping_t *map = NULL;
    proto_tree *item = NULL;
    struct mptcp_analysis *mptcpd = tcpd->mptcp_analysis;

    if(!mptcp_interdsn_latency) {
        return;
    }

    if(tcpd->fwd->mptcp_subflow->meta->base_dsn == tcph->th_mptcp->mh_dsn)
    {
        return;
    }

    /* if first time searching */
    if(!mptcppd->previous_packet_dsn) {

        guint64 requested_dsn = tcph->th_mptcp->mh_rawdsn64 - 1;

        /* look for the packet with the DSN just before this packet dsn */
        wmem_list_frame_t *subflow_it = NULL;

        /* results should be some kind of  in case 2 DSS are needed to cover this packet */
        for(subflow_it = wmem_list_head(mptcpd->subflows);
            subflow_it != NULL; subflow_it = wmem_list_frame_next(subflow_it))
        {
            ...
            results = wmem_itree_find_intervals(
                    sf->dsn_map, wmem_packet_scope(), requested_dsn, requested_dsn);

            for(packet_it = wmem_list_head(results);
                packet_it != NULL;
                packet_it = wmem_list_frame_next(packet_it))
            {
                mptcp_dsn2packet_mapping_t *temp = (mptcp_dsn2packet_mapping_t *)wmem_list_frame_data(packet_it);
                if(!map) {
                    map = temp;
                }
                else if( (temp->frame > map->frame) && temp->frame < PINFO_FD_NUM(pinfo)) {
                    map = temp;
                }
            }
      }
        ...
        map = mptcppd->previous_packet_dsn;
    }
    ...
    if(!map) {
        ...
        mptcppd->previous_packet_dsn = map;
    }

    DISSECTOR_ASSERT(map);

    {
        ...
        if(!t2) {
            ...
            item = proto_tree_add_time(tree, hf_mptcp_application_latency, tvb, 0, 0, &delta);
            PROTO_ITEM_SET_GENERATED(item);
...
)
{
        ...
        packet_it = wmem_list_frame_next(packet_it))
    {
        ...
        proto_tree_add_uint(tree, hf_mptcp_duplicated_data, tvb, 0, 0, packet->frame);
    }
...
)
{
        ...
        dss_it= wmem_list_frame_next(dss_it))
    {
        ...
        proto_tree_add_uint(tree, hf_mptcp_related_mapping, tvb, 0, 0, mapping->frame);
    }
    ...
    */
    ...
    if(mptcp_convert_dsn(rawdsn, tcpd->fwd->mptcp_subflow->meta,
        ...
        if (!PINFO_FD_VISITED(pinfo))
        {
            ...
            mptcppd->previous_packet_dsn = packet;
        }
        PROTO_ITEM_SET_GENERATED(item);
                        ...
                        )
{
    ...
    /* Print elpased time between current bytes and immediate previous byte */
    mptcp_dsn_add_latency(pinfo, tvb, tree, tcpd, tcph, mptcppd);
...
proto_register_tcp(void)
{
    ...
    prefs_register_bool_preference(mptcp_module, "interdsn_latency",
        "Interarrival of DSN",
        "You need to enable DSS mapping analysis for this option to work",
        &mptcp_interdsn_latency);
